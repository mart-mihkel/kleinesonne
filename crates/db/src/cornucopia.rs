...
impl<'a> From<DogBorrowed<'a>> for Dog
{
    ...
    Self { Self { id,name: name.into(),nickname: nickname.into(),father: father.into(),mother: mother.into(),breed,gender,dob,alive,thumbnail: thumbnail.map(|v| v.into()),images: images.map(|v| v.into()).collect(),health: health.map(|v| v.into()).collect(),titles: titles.map(|v| v.into()).collect(),} }
...
} }pub fn alive_dogs_by_breed_and_gender() -> AliveDogsByBreedAndGenderStmt
{ AliveDogsByBreedAndGenderStmt(cornucopia_async::private::Stmt::new("SELECT
	id,
	name,
	nickname,
	father,
...
AND
	gender = $2
AND
	alive = true")) } pub struct
AliveDogsByBreedAndGenderStmt(cornucopia_async::private::Stmt); impl AliveDogsByBreedAndGenderStmt
{ pub fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
breed: &'a super::super::types::public::Breed,gender: &'a super::super::types::public::Gender,) -> DogQuery<'a,C,
Dog, 2>
{
    DogQuery
    {
        client, params: [breed,gender,], stmt: &mut self.0, extractor:
        |row| { DogBorrowed { id: row.get(0),name: row.get(1),nickname: row.get(2),father: row.get(3),mother: row.get(4),breed: row.get(5),gender: row.get(6),dob: row.get(7),alive: row.get(8),thumbnail: row.get(9),images: row.get(10),health: row.get(11),titles: row.get(12),} }, mapper: |it| { <Dog>::from(it) },
    }
} }impl <'a, C: GenericClient,> cornucopia_async::Params<'a,
AliveDogsByBreedAndGenderParams<>, DogQuery<'a, C,
Dog, 2>, C> for AliveDogsByBreedAndGenderStmt
{
    fn
    params(&'a mut self, client: &'a  C, params: &'a
    AliveDogsByBreedAndGenderParams<>) -> DogQuery<'a, C,
    Dog, 2>
    { self.bind(client, &params.breed,&params.gender,) }
}pub fn retired_dogs_by_breed() -> RetiredDogsByBreedStmt
{ RetiredDogsByBreedStmt(cornucopia_async::private::Stmt::new("SELECT
	id,
	name,
	nickname,
	father,
...
WHERE
	breed = $1
AND
	alive = false")) } pub struct
RetiredDogsByBreedStmt(cornucopia_async::private::Stmt); impl RetiredDogsByBreedStmt
{ pub fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
breed: &'a super::super::types::public::Breed,) -> DogQuery<'a,C,
Dog, 1>
{
    DogQuery
    {
        client, params: [breed,], stmt: &mut self.0, extractor:
        |row| { DogBorrowed { id: row.get(0),name: row.get(1),nickname: row.get(2),father: row.get(3),mother: row.get(4),breed: row.get(5),gender: row.get(6),dob: row.get(7),alive: row.get(8),thumbnail: row.get(9),images: row.get(10),health: row.get(11),titles: row.get(12),} }, mapper: |it| { <Dog>::from(it) },
    }
...
name: &'a T1,nickname: &'a T2,father: &'a T3,mother: &'a T4,breed: &'a super::super::types::public::Breed,gender: &'a super::super::types::public::Gender,dob: &'a i64,alive: &'a bool,thumbnail: &'a Option<T5>,images: &'a T7,health: &'a T9,titles: &'a T11,) -> I32Query<'a,C,
i32, 12>
{
    I32Query
    {
...
cornucopia_async::StringSql,T11:
cornucopia_async::ArraySql<Item = T10>,>(&'a mut self, client: &'a  C,
name: &'a T1,nickname: &'a T2,father: &'a T3,mother: &'a T4,breed: &'a super::super::types::public::Breed,gender: &'a super::super::types::public::Gender,dob: &'a i64,alive: &'a bool,thumbnail: &'a Option<T5>,images: &'a T7,health: &'a T9,titles: &'a T11,id: &'a i32,) -> Result<u64, tokio_postgres::Error>
{
...
}pub fn delete_dog_thumbnail() -> DeleteDogThumbnailStmt
{ DeleteDogThumbnailStmt(cornucopia_async::private::Stmt::new("UPDATE
	dogs
SET
	thumbnail = null
WHERE
	id = $1")) } pub struct
DeleteDogThumbnailStmt(cornucopia_async::private::Stmt); impl DeleteDogThumbnailStmt
{ pub async fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
...
} }pub fn delete_dog_image() -> DeleteDogImageStmt
{ DeleteDogImageStmt(cornucopia_async::private::Stmt::new("UPDATE
	dogs
SET
	images = array_remove(images, $1)
WHERE
	id = $2")) } pub struct
DeleteDogImageStmt(cornucopia_async::private::Stmt); impl DeleteDogImageStmt
{ pub async fn bind<'a, C:
GenericClient,T1:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
image: &'a T1,id: &'a i32,) -> Result<u64, tokio_postgres::Error>
{
    let stmt = self.0.prepare(client).await?;
    client.execute(stmt, &[image,id,]).await
} }impl <'a, C: GenericClient + Send + Sync, T1: cornucopia_async::StringSql,>
cornucopia_async::Params<'a, DeleteDogImageParams<T1,>, std::pin::Pin<Box<dyn futures::Future<Output = Result<u64,
tokio_postgres::Error>> + Send + 'a>>, C> for DeleteDogImageStmt
{
    fn
    params(&'a mut self, client: &'a  C, params: &'a
    DeleteDogImageParams<T1,>) -> std::pin::Pin<Box<dyn futures::Future<Output = Result<u64,
    tokio_postgres::Error>> + Send + 'a>>
    { Box::pin(self.bind(client, &params.image,&params.id,)) }
...
{ use futures::{{StreamExt, TryStreamExt}};use futures; use cornucopia_async::GenericClient;#[derive( Debug)] pub struct InsertLitterParams<T1: cornucopia_async::StringSql,T2: cornucopia_async::StringSql,T3: cornucopia_async::StringSql,T4: cornucopia_async::ArraySql<Item = T3>,> { pub name: T1,pub dob: i64,pub breed: super::super::types::public::Breed,pub parents_image: Option<T2>,pub images: T4,}#[derive( Debug)] pub struct UpdateLitterParams<T1: cornucopia_async::StringSql,T2: cornucopia_async::StringSql,T3: cornucopia_async::StringSql,T4: cornucopia_async::ArraySql<Item = T3>,> { pub name: T1,pub dob: i64,pub breed: super::super::types::public::Breed,pub parents_image: Option<T2>,pub images: T4,pub id: i32,}#[derive( Debug)] pub struct DeleteLitterImageParams<T1: cornucopia_async::StringSql,> { pub image: T1,pub id: i32,}#[derive(serde::Serialize, Debug, Clone, PartialEq,)] pub struct AllNames
{ pub id : i32,pub name : String,}pub struct AllNamesBorrowed<'a> { pub id : i32,pub name : &'a str,}
impl<'a> From<AllNamesBorrowed<'a>> for AllNames
{
    fn from(AllNamesBorrowed { id,name,}: AllNamesBorrowed<'a>) ->
...
{ pub id : i32,pub name : String,pub dob : i64,pub breed : super::super::types::public::Breed,pub parents_image : Option<String>,pub images : Vec<String>,}pub struct LitterBorrowed<'a> { pub id : i32,pub name : &'a str,pub dob : i64,pub breed : super::super::types::public::Breed,pub parents_image : Option<&'a str>,pub images : cornucopia_async::ArrayIterator<'a, &'a str>,}
impl<'a> From<LitterBorrowed<'a>> for Litter
{
    fn from(LitterBorrowed { id,name,dob,breed,parents_image,images,}: LitterBorrowed<'a>) ->
    Self { Self { id,name: name.into(),dob,breed,parents_image: parents_image.map(|v| v.into()),images: images.map(|v| v.into()).collect(),} }
...
Litter, 1>
{
    ...
    {
        client, params: [id,], stmt: &mut self.0, extractor:
        |row| { LitterBorrowed { id: row.get(0),name: row.get(1),dob: row.get(2),breed: row.get(3),parents_image: row.get(4),images: row.get(5),} }, mapper: |it| { <Litter>::from(it) },
    }
...
FROM
	litters l
WHERE
	(SELECT count(id) FROM puppies WHERE litter_id = l.id and availability in ('Available', 'Co')) > 0")) } pub struct
...
AND
	(SELECT count(id) FROM puppies WHERE litter_id = l.id and availability in ('Available', 'Co')) > 0")) } pub struct
AvailableLittersByBreedStmt(cornucopia_async::private::Stmt); impl AvailableLittersByBreedStmt
{ pub fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
breed: &'a super::super::types::public::Breed,) -> LitterQuery<'a,C,
Litter, 1>
{
    LitterQuery
    {
...
name: &'a T1,dob: &'a i64,breed: &'a super::super::types::public::Breed,parents_image: &'a Option<T2>,images: &'a T4,) -> I32Query<'a,C,
i32, 5>
{
    I32Query
    {
        client, params: [name,dob,breed,parents_image,images,], stmt: &mut self.0, extractor:
        |row| { row.get(0) }, mapper: |it| { it },
    }
...
i32, 5>, C> for InsertLitterStmt
{
    fn
    params(&'a mut self, client: &'a  C, params: &'a
    InsertLitterParams<T1,T2,T3,T4,>) -> I32Query<'a, C,
    i32, 5>
    { self.bind(client, &params.name,&params.dob,&params.breed,&params.parents_image,&params.images,) }
...
cornucopia_async::StringSql,T4:
cornucopia_async::ArraySql<Item = T3>,>(&'a mut self, client: &'a  C,
name: &'a T1,dob: &'a i64,breed: &'a super::super::types::public::Breed,parents_image: &'a Option<T2>,images: &'a T4,id: &'a i32,) -> Result<u64, tokio_postgres::Error>
{
    let stmt = self.0.prepare(client).await?;
    client.execute(stmt, &[name,dob,breed,parents_image,images,id,]).await
...
tokio_postgres::Error>> + Send + 'a>>, C> for UpdateLitterStmt
{
    ...
    tokio_postgres::Error>> + Send + 'a>>
    { Box::pin(self.bind(client, &params.name,&params.dob,&params.breed,&params.parents_image,&params.images,&params.id,)) }
...
WHERE
	id = $2")) } pub struct
DeleteLitterImageStmt(cornucopia_async::private::Stmt); impl DeleteLitterImageStmt
{ pub async fn bind<'a, C:
GenericClient,T1:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
image: &'a T1,id: &'a i32,) -> Result<u64, tokio_postgres::Error>
...
cornucopia_async::Params<'a, DeleteLitterImageParams<T1,>, std::pin::Pin<Box<dyn futures::Future<Output = Result<u64,
tokio_postgres::Error>> + Send + 'a>>, C> for DeleteLitterImageStmt
{
    fn
    params(&'a mut self, client: &'a  C, params: &'a
    DeleteLitterImageParams<T1,>) -> std::pin::Pin<Box<dyn futures::Future<Output = Result<u64,
    tokio_postgres::Error>> + Send + 'a>>
    { Box::pin(self.bind(client, &params.image,&params.id,)) }
...
} }pub fn article_by_id() -> ArticleByIdStmt
{ ArticleByIdStmt(cornucopia_async::private::Stmt::new("SELECT
	id,
	title,
	text,
	date,
...
WHERE
	id = $1")) } pub struct
ArticleByIdStmt(cornucopia_async::private::Stmt); impl ArticleByIdStmt
{ pub fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
id: &'a i32,) -> ArticleQuery<'a,C,
Article, 1>
{
    ArticleQuery
    {
...
}pub fn delete_article_image() -> DeleteArticleImageStmt
{ DeleteArticleImageStmt(cornucopia_async::private::Stmt::new("UPDATE
	news
SET
	images = array_remove(images, $1)
WHERE
	id = $2")) } pub struct
DeleteArticleImageStmt(cornucopia_async::private::Stmt); impl DeleteArticleImageStmt
{ pub async fn bind<'a, C:
GenericClient,T1:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
image: &'a T1,id: &'a i32,) -> Result<u64, tokio_postgres::Error>
...
cornucopia_async::Params<'a, DeleteArticleImageParams<T1,>, std::pin::Pin<Box<dyn futures::Future<Output = Result<u64,
tokio_postgres::Error>> + Send + 'a>>, C> for DeleteArticleImageStmt
{
    fn
    params(&'a mut self, client: &'a  C, params: &'a
    DeleteArticleImageParams<T1,>) -> std::pin::Pin<Box<dyn futures::Future<Output = Result<u64,
    tokio_postgres::Error>> + Send + 'a>>
    { Box::pin(self.bind(client, &params.image,&params.id,)) }
...
{ use futures::{{StreamExt, TryStreamExt}};use futures; use cornucopia_async::GenericClient;#[derive( Debug)] pub struct InsertPuppyParams<T1: cornucopia_async::StringSql,T2: cornucopia_async::StringSql,> { pub litter_id: i32,pub name: T1,pub gender: super::super::types::public::Gender,pub availability: super::super::types::public::Availability,pub image: Option<T2>,}#[derive( Debug)] pub struct UpdatePuppyParams<T1: cornucopia_async::StringSql,T2: cornucopia_async::StringSql,> { pub litter_id: i32,pub name: T1,pub gender: super::super::types::public::Gender,pub availability: super::super::types::public::Availability,pub image: Option<T2>,pub id: i32,}#[derive(serde::Serialize, Debug, Clone, PartialEq,)] pub struct NamesByLitter
...
}#[derive(serde::Serialize, Debug, Clone, PartialEq,)] pub struct Puppy
{ pub id : i32,pub litter_id : i32,pub name : String,pub gender : super::super::types::public::Gender,pub availability : super::super::types::public::Availability,pub image : Option<String>,}pub struct PuppyBorrowed<'a> { pub id : i32,pub litter_id : i32,pub name : &'a str,pub gender : super::super::types::public::Gender,pub availability : super::super::types::public::Availability,pub image : Option<&'a str>,}
impl<'a> From<PuppyBorrowed<'a>> for Puppy
{
    fn from(PuppyBorrowed { id,litter_id,name,gender,availability,image,}: PuppyBorrowed<'a>) ->
    Self { Self { id,litter_id,name: name.into(),gender,availability,image: image.map(|v| v.into()),} }
}pub struct PuppyQuery<'a, C: GenericClient, T, const N: usize>
{
    client: &'a  C, params:
    [&'a (dyn postgres_types::ToSql + Sync); N], stmt: &'a mut
    cornucopia_async::private::Stmt, extractor: fn(&tokio_postgres::Row) -> PuppyBorrowed,
    mapper: fn(PuppyBorrowed) -> T,
} impl<'a, C, T:'a, const N: usize> PuppyQuery<'a, C, T, N> where C:
GenericClient
{
    pub fn map<R>(self, mapper: fn(PuppyBorrowed) -> R) ->
    PuppyQuery<'a,C,R,N>
    {
        PuppyQuery
        {
...
} }pub fn puppy_by_id() -> PuppyByIdStmt
{ PuppyByIdStmt(cornucopia_async::private::Stmt::new("SELECT
	id,
	litter_id,
	name,
	gender,
...
WHERE
	id = $1")) } pub struct
PuppyByIdStmt(cornucopia_async::private::Stmt); impl PuppyByIdStmt
{ pub fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
id: &'a i32,) -> PuppyQuery<'a,C,
Puppy, 1>
{
    PuppyQuery
    {
        client, params: [id,], stmt: &mut self.0, extractor:
        |row| { PuppyBorrowed { id: row.get(0),litter_id: row.get(1),name: row.get(2),gender: row.get(3),availability: row.get(4),image: row.get(5),} }, mapper: |it| { <Puppy>::from(it) },
    }
...
{ pub fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
litter_id: &'a i32,) -> PuppyQuery<'a,C,
Puppy, 1>
{
    PuppyQuery
    {
...
cornucopia_async::StringSql,T2:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
litter_id: &'a i32,name: &'a T1,gender: &'a super::super::types::public::Gender,availability: &'a super::super::types::public::Availability,image: &'a Option<T2>,) -> I32Query<'a,C,
i32, 5>
{
    I32Query
    {
...
cornucopia_async::StringSql,T2:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
litter_id: &'a i32,name: &'a T1,gender: &'a super::super::types::public::Gender,availability: &'a super::super::types::public::Availability,image: &'a Option<T2>,id: &'a i32,) -> Result<u64, tokio_postgres::Error>
{
    let stmt = self.0.prepare(client).await?;
    client.execute(stmt, &[litter_id,name,gender,availability,image,id,]).await
...
tokio_postgres::Error>> + Send + 'a>>, C> for UpdatePuppyStmt
{
    ...
    tokio_postgres::Error>> + Send + 'a>>
    { Box::pin(self.bind(client, &params.litter_id,&params.name,&params.gender,&params.availability,&params.image,&params.id,)) }
}pub fn delete_puppy_image() -> DeletePuppyImageStmt
{ DeletePuppyImageStmt(cornucopia_async::private::Stmt::new("UPDATE
	puppies
SET
	image = null
WHERE
	id = $1")) } pub struct
DeletePuppyImageStmt(cornucopia_async::private::Stmt); impl DeletePuppyImageStmt
{ pub async fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
id: &'a i32,) -> Result<u64, tokio_postgres::Error>
{
